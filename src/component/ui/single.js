...
const Single = () => {
  ...
  const ModalUseOpenBtn = useRef();
  const ModalUseCloseBtn = useRef();
  ...
  const getFacilitySlug = async ({ slug }) => {
    await axios
      .get(`${BASE_URL}/get-facility-by-slug/${slug}`)
      .then((response) => {
        if (response.data.facility) {
          let imagesCopy = [response.data.facility.featured_image];

          if (
            response.data.facility.services &&
            response.data.facility.services.length > 0
          ) {
  ...
  const onSubmit = async(data) => {
    ...
    if (selectedService) {
      const getIndex =
        ...
        facility.services.findIndex((item) => item.id == selectedService);
      const getSelectedService = { ...facility?.services[getIndex] };
      services = { ...getSelectedService };

      const getCourtIndex =
        getSelectedService &&
        getSelectedService?.court &&
        Array.isArray(getSelectedService?.court) &&
        getSelectedService.court.length > 0 &&
        getSelectedService.court.findIndex((item) => item.id == courtId);
      const getCourt = { ...getSelectedService?.court[getCourtIndex] };
      court = { ...getCourt };
    }
    ...
    const requestBody = {
      ...
      duration: court && court?.duration,
      end_time: court && court?.end_time,
      total_price: court && court?.slot_price,
      court_id: courtId,
      start_time: court && court?.start_time,
      name: data.name,
      email: data.email,
    ...
    const response = await axiosAuth.post(`${BASE_URL}/booking`, requestBody);
    ...
    if (response) {
      ...
      toast.current.show({
        severity: "success",
        summary: "Success",
        detail: `${(response.message, "Address updated sucessfully.")}`,
        life: 2000,
      });
      resetForm();
    }
  ...
  const handleToBookNow = () => {
    if (
      ...
      facility.length > 0 &&
      ModalUseOpenBtn &&
      ModalUseCloseBtn.current
    ) {
      return ModalUseOpenBtn.current.click();
    }
    if (ModalUseOpenBtn && ModalUseOpenBtn.current)
  ...
  const itemTemplate = (item) => {
    ...
    return (
      <div className="carousel-img-wrapper-si">
        <img
          src={`${IMG_URL}${imgUrl}`}
          alt={facility.official_name}
          className="carousel-img-si img-fluid"
          loading={lazy}
        />
  ...
  const checkServiceDisabled = (id) => {
    ...
    if (exists) {
      ...
      const formattedDateToCheck = `${("0" + dateToCheck.getDate()).slice(
        -2
      )}-${("0" + (dateToCheck.getMonth() + 1)).slice(
        -2
  ...
  const handleCourtChange = async (courtId) => {
    ...
    ).slice(-2)}-${("0" + bookingDate.getDate()).slice(-2)}`;
    let formData = {
      court_id: courtId,
  ...
  const handleSlotSelection = async (index) => {
  ...
  const initiatePayment = (data) => {
    ...
    const userToken = Cookies.get("USER_TOKEN");
     ...
     const options = {
      ...
      image: "https://bookvenue.app/logo192.png",
      order_id: data.order.id,
        handler: async(response) =>{
          try {
            setVerifingPayment(true);
            const apiResponse = await axiosAuth.post(`${BASE_URL}/payment-success`,{
              "order_id" : data && data?.order?.id 
            });
            if (apiResponse.statusText == "OK") {
              setOrderId(apiResponse?.data?.payment?.id)
              
            if (ModalUseOpenBtn.current) {
              ModalUseOpenBtn.current.click();
            }
    ...
    rzp1.on('payment.failed', async function (response) { 
      try {
        await axiosAuth.get(`${BASE_URL}/payment-failure`, 
        {
          params: { "order_id": data && data?.order?.id }
        }
      )
      }
      catch (error) {
              ...
              <section className="container single-page-container mt-3">
                <div className="row mb-5">
                  <div className="col-12">
                    <h3 className="title-heading-si m-0">
                  ...
                  <div className="col-8 img-carousel-container-si">
                    ...
                    <div className="card mt-4">
                      <div className="d-flex px-4 py-3 align-items-center described-si">
                        ...
                        <span className="mx-4 rating-text-si">
                          (18 ratings)
                        </span>
                        <span>
                          <a href="#" className="rate-venue-si">
                            Rate Venue
                          </a>
                        </span>
                      </div>
                    ...
                    <div className="mt-5 nav-tabs-si">
                      ...
                      <div className="tab-content" id="singlePageTabContent">
                        ...
                        >
                          <div className="tab-container-si">
                            <h4 className="heading-si text-capitalize">
                              {facility.category}
                            </h4>
                            {facility.services.length > 0 ? (
                              <>
                        ...
                        >
                          <div className="tab-container-si">
                            ...
                            <p className="mb-3 desc-si">
                              ...
                              <span className="text-si">Washroom</span>
                            ...
                            <p className="mb-3 desc-si">
                              ...
                              <span className="text-si">Parking Area</span>
                              <span className="ms-2 icon-si">
                                <svg
                                  width="20"
                                  height="20"
                  ...
                  <div className="col-4">
                    <div className="configure-container-si">
                      ...
                      {value ? (
                        <>
                          <div className="card p-4 mb-3 px-4">
                            <p className="heading-si text-capitalize">
                                ...
                                <>
                                  {facility.services.map((item, ind) => {
                                    ...
                                    return (
                                      <button
                                        type="button"
                                        className={`btn choose-service-btn-si m-1  ${
                                        ...
                                        disabled={checkServiceDisabled(item.id)}
                                      >
                                ...
                                {facility.services
                                  ...
                                  .court.map((items, ind) => {
                                    return (
                                      <button
                                        type="button"
                                        className={`btn choose-courts-btn-si ${
                                          courtId === items.id ? "active" : ""
                                        } `}
                                        key={items.court_name + ind}
                                        onClick={() =>
                                          handleCourtChange(items.id)
                                        }
                                      >
                                        <p className="mb-1 text-start headingSi">
                                          {items.court_name}
                                        </p>
                                        <p className="mb-1 text-start">
                                          Slot time: {items.duration} min
                                        </p>
                                        <p className="mb-1 text-start">
                                          Slot price: {items.slot_price} ₹
                                        </p>
                                      </button>
                                    );
                                  })}
                      ...
                      {slots && courtId ? (
                        <>
                          <div className="card p-4 mb-3 px-4 singlePage-slots-wrapper">
                            <p className="heading-si text-capitalize w-100">
                              Choose slot{" "}
                            </p>
                            {slots.length > 0 ? (
                              <>
                                {slots.map((item, ind) => {
                                  return (
                                    <button
                                      type="button"
                                      className={`btn choose-courts-btn-si m-2 ${
                                        ind === selectedSlot ? "active" : ""
                                      }`}
                                      key={item.start_time + ind}
                                      onClick={() => handleSlotSelection(ind)}
                                    >
                                      <p className="mb-1 text-start headingSi">
                      ...
                      {!state.userData ? (
                        <button
                          type="button"
                          className="btn book-now-btn-si mt-2"
                        ...
                        <button
                          type="button"
                          className="btn book-now-btn-si mt-2"
                          onClick={() => handleToBookNow()}
                          disabled={!(value && courtId && selectedService)}
                        >
                          Book Now
                        </button>
                      )}
                ...
                <button
                  ...
                  type="button"
                  data-bs-toggle="modal"
                  data-bs-target="#useThisAddressModalToggle"
                  className="d-none"
                ></button>
                <button
                  ref={ModalUseCloseBtn}
                  type="button"
                  data-bs-toggle="modal"
                  data-bs-target="#locationModalToggle"
                  className="d-none"
                ></button>

                {/* Use This Address Modal */}
                <div
                  class="modal modal-lg fade usr-artist-single-modal"
                  id="useThisAddressModalToggle"
                  data-bs-backdrop="static"
                  data-bs-keyboard="false"
                  tabindex="-1"
                  aria-labelledby="useThisAddressModalToggleLabel"
                  aria-hidden="true"
                >
                    ...
                    <div class="modal-content">
                      <div class="modal-body">
                        <div className="modal-btn-wrapper justify-content-end d-flex">
                          <h3 className="text-center mt-3 text-uppercase w-100">
                            Select Your Address
                          </h3>
                          <button
                            type="button"
                            class=" common-modal-dismiss-btn"
